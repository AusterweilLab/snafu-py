...
# - todo: return truncated path. actually can remove first hit node IF it already appears before FH(x+1)
...
# given a random walk Z, generate new Z using Gibbs sampling
def gibbZ(x,a,sweeps=10):
    seed=path_from_walk(genZfromX(x,a))          # markov chain starting at random node
    
    while True:
        for i in range(sweeps):
            order=range(len(seed))[2:-1]        # can't change first, second, or last element of walk without changing X
            random.shuffle(order)               
            for j in order:

                # must link to nodes before and after
                possibles=np.intersect1d(np.nonzero(a[seed[j-1]])[0],np.nonzero(a[seed[j+1]])[0])
                # must satisfy X
                prev=x[:x.index(seed[j])+2]                   # this is called 'possibles'in genZfromX
                if seed[j] not in seed[:j]:
                    if seed[j] not in seed[j+1:seed.index(x[x.index(seed[j])+1])]:
                        prev=[seed[j]]            # cant change first hit node unless it already appears before FH(x+1)
                possibles=np.intersect1d(possibles,prev)
