...
for subj in subs:
    ...
    # windowgraph_valid
    window_graph_valid=rw.windowGraph(Xs, numnodes, td=toydata, valid=1, fitinfo=fitinfo)
    
    g=nx.to_networkx_graph(window_graph)
    g2=nx.to_networkx_graph(window_graph_valid)

    nx.relabel_nodes(g, items, copy=False)
    nx.relabel_nodes(g2, items, copy=False)
    
    rw.write_csv([g, g2],subj+"w.csv",subj) # write multiple graphs
