...
usf_graph, usf_items = rw.read_csv("USF_animal_subset.snet")
usf_graph_nx = nx.from_numpy_matrix(usf_graph)
usf_numnodes = len(usf_items)

maxsubs=50
numsubs = int(sys.argv[2])+1
skips = numsubs / maxsubs # how many simulated data sets to skip
numsubs = numsubs % maxsubs

numlists = 3
listlength = 35
numsims = 1
#methods=['rw','goni','chan','kenett','fe']
method="uinvite_hierarchical"
...
seednum += skips*(numsubs*numlists)

outfile=method + "_" + str(skips) + "_" + str(numsubs) + ".csv"
with open(outfile,'w',0) as fh:
    ...
    for simnum in range(numsims):
        ...
        for method in methods:
            ...
            fh.write(method + "," + str(skips) + "," + str(listnum))
            for i in costlist:
                fh.write("," + str(i))
