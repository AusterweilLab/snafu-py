...
def spreadingActivationSearch(g, td, decay):
    ...
    activations=dict.fromkeys(range(numnodes), 0)

    activations[start]=1.0
    walk=[start]
    
    while len(walk) < numtrim:
        newacts=dict.fromkeys(range(numnodes), 0)
        walknodes=[]
        ...
        newnode=np.random.choice(range(numnodes),p=probs)
        walk.append(newnode)
