---
title: Reconstructing semantic networks
output: pdf_document
---
...
The following simulations use a sample of 100 graphs with 5 observed lists.
Except when otherwise noted, the comparisons use exactly the same true graph
and the same lists (Xs). For each simulation, all other parameters except the
test parameter are being held constant.  The true graphs were generated using a
small-world process and contain 15 nodes and 30 edges.
...
The cost with each method reflects the number of edges changes (additions or
...
converging, so I tried two tolerance parameters. Thankfully, the process
converges with a relatively small tolerance value, meaning I can run these
simulations in hours and not days. However it's likely that if the graph size
were increased, the search parameters will need to be altered to ensure that it
explores the space appropriately. Currently this process is still very ad hoc.
...
the graph using IRTs actually got a tiny bit worse with a higher tolerance.
Note that our methods perform better than RW, but using IRTs actually makes the
graph fit worse; I'll get to that later.  Here are the distributions, excluding
the "RW" method but collapsing over IRT and no-IRT graphs.

```{r}
    converge_dat<-melt(dat[numx==5 & jeff==0.5 & beta==0.9,],measure.vars=c("cost_orig","cost_irt","cost_noirt"))
    ggplot(converge_dat[variable!="cost_orig"],aes(x=value, fill=factor(max_converge))) + geom_density(alpha=0.4)
```
...
The method performs reasonably well with a beta value or 0.9. These
distributions show that the beta value has a pretty small effect, at least
within this range.

```{r}
    beta_dat<-melt(dat[numx==5 & jeff==0.5 & max_converge==5,],measure.vars=c("cost_orig","cost_irt","cost_noirt"))
    ggplot(beta_dat[variable=="cost_irt"],aes(x=value, fill=factor(beta))) + geom_density(alpha=0.4)
```

## IRT weighting

One problem is that our method uses a weighting parameter combine IRT and
...
```{r}
    ...
    ggplot(jeff_dat[variable=="cost_irt"],aes(x=value, fill=factor(jeff))) + geom_density(alpha=0.4)
```
...
```{r}
    ...
    ggplot(compare_dat,aes(x=value, fill=factor(variable))) + geom_density(alpha=0.4)
```

The answer is that it buys you a little, but not a lot. This advantage may
improve with more lists, we should try even lower weighting parameters. But
it's nice to know our method does something.
...
You can see the RW distribution has been pushed closer to the other
distributions:
...
These distributions compare each type before and after truncating the lists:

```{r}
    compare_dat<-melt(dat[numx==5 & max_converge==5 & jeff==0.9 & beta==0.9,],measure.vars=c("cost_orig","cost_irt","cost_noirt"))
    orig<-ggplot(compare_dat[variable=="cost_orig"],aes(x=value, fill=factor(trim))) + geom_density(alpha=0.4) + ggtitle("RW") + theme(legend.position="bottom")
    irt<-ggplot(compare_dat[variable=="cost_irt"],aes(x=value, fill=factor(trim))) + geom_density(alpha=0.4) + ggtitle("w/ IRT") + theme(legend.position="bottom")
    noirt<-ggplot(compare_dat[variable=="cost_noirt"],aes(x=value, fill=factor(trim))) + geom_density(alpha=0.4) + ggtitle("w/o IRT") + theme(legend.position="bottom")
...
```{r}
    ...
    one<-ggplot(hit_dat,aes(x=value, fill=factor(variable))) + geom_density(alpha=0.4) + theme(legend.position="bottom")

    fa_dat<-melt(dat[numx==5 & jeff==0.9 & beta==0.9 & trim==1 & max_converge==5,],measure.vars=c("fa_orig","fa_irt","fa_noirt"))
    two<-ggplot(fa_dat,aes(x=value, fill=factor(variable))) + geom_density(alpha=0.4) + theme(legend.position="bottom")
...
```{r}
    ...
    ggplot(numx_dat,aes(y=value,x=numx,color=variable)) + geom_point() + geom_line() + ylim(0,35)
```
...
```{r}
    ...
    sdt_dat[,c("sdt","type"):=tstrsplit(variable,"_")]
    sdt_dat<-dcast(sdt_dat, sdt ~ type)
    sdt_dat<-dcast(sdt_dat, numx + type ~ sdt ,value.var="value")
    sdt_dat[,fa:=fa/75]
    sdt_dat[,hit:=hit/30]
